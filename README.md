...
### V2 Performance (Scaffold Split)

| Endpoint | Metric | Random Split | Scaffold Split | Dataset Size |
|----------|--------|--------------|----------------|--------------|
| hERG | R² | 0.57 | 0.38 | 10,661 |
| Hepatotox | AUC-ROC | 0.80 | 0.76 | 1,511 |
| Nephrotox | AUC-ROC | 0.91 | 0.83 | 302 |
| Ames | AUC-ROC | 0.92 | 0.84 | 876 |
| Skin Sens | AUC-ROC | 0.87 | 0.82 | 423 |
| Cytotox | AUC-ROC | 0.89 | 0.90 | 387 |
| Reproductive Tox* | AUC-ROC | 0.77 | 0.59 | 127 |

> **Note on Scaffold Split:** Scaffold-based validation provides a more realistic estimate of model performance on novel chemical series. The ~10% performance drop compared to random splits is expected and indicates models generalize to novel scaffolds.

> **Note on Reproductive Toxicity:** This endpoint has a limited training dataset (127 compounds) and is excluded from the integrated risk score calculation due to insufficient predictive power. Predictions should be interpreted with caution.

---

## V2 Enhancements

### Enhanced 3D Descriptors
- **1,975 features** from multi-conformer ensembles (5 conformers per molecule)
- Shape descriptors with aggregation (mean/std/min/max)
- AUTOCORR3D, RDF, WHIM, and ErG fingerprints
- Located in `src/feature_engineering/descriptors_3d_enhanced.py`

### Scaffold-Split Validation
- **GroupKFold** using Murcko scaffold as group identifier
- Ensures entire scaffolds are in train OR test, never both
- More realistic estimate of performance on novel chemical series
- Located in `src/validation/scaffold_split.py`

### Multi-Task Learning
- **Shared representation** learning across 6 classification endpoints
- Masked loss handles missing labels per endpoint
- Per-task class weights for imbalanced datasets
- PyTorch neural network (optional) or XGBoost stacking fallback
- Located in `src/models/multitask_model.py`

### Integrated Risk Scoring
- **Confidence-weighted aggregation** across all endpoints
- Endpoint-specific weights based on clinical importance
- Sigmoid transformation for hERG pIC50 (midpoint=6.0)
- Risk categories: LOW (<0.3), MODERATE (0.3-0.6), HIGH (>0.6)
- Validation: Mann-Whitney p<0.001, AUC=0.81 on toxic vs safe compounds
- Located in `src/scoring/risk_score.py`

### Running V2 Experiments

```bash
# Enhanced 3D descriptors
python src/feature_engineering/descriptors_3d_enhanced.py --endpoints herg

# Scaffold-split validation
python src/experiments/scaffold_validation.py --endpoints herg hepatotox ames

# Multi-task learning
python src/experiments/multitask_experiment.py

# Risk score validation
python src/experiments/risk_score_validation.py
```
